...
// Sevier County GIS (hosted by City of Sevierville, carries countywide CAMA data)
const SEVIER_MAP =
  "https://gis.seviervilletn.org/arcgis/rest/services/PublicMaps/Sevierville_Zoning_Map/MapServer";
const SEVIER_LAYER = { zoning: 0, parcels: 3, cityLimits: 4, interstateOverlay: 12 };
...
export async function sevierCama(lat, lng) {
  try {
    const feats = await arcgisQuery(
      `${SEVIER_MAP}/${SEVIER_LAYER.parcels}/query`,
      pointParams(lat, lng)
    );
...
export async function sevierJurisdiction(lat, lng) {
  const out = { inCityLimits: null, overlayDistrict: null };
  try {
    const city = await arcgisQuery(
      `${SEVIER_MAP}/${SEVIER_LAYER.cityLimits}/query`,
      pointParams(lat, lng, "*")
    );
    out.inCityLimits = city.length > 0;
  } catch {
    /* leave null = unknown */
  }
  try {
    const ov = await arcgisQuery(
      `${SEVIER_MAP}/${SEVIER_LAYER.interstateOverlay}/query`,
      pointParams(lat, lng, "*")
    );
    out.overlayDistrict = ov.length > 0 ? "Interstate Overlay District" : null;
  } catch {
    /* leave null */
  }
  return out;
 ...
 */
export function buildFacts({ lp, cama, jurisdiction, countyName }) {
  const county = countyName || lp?.situsCounty || null;
  const isSevier = /sevier/i.test(county || "");
  const assessor = `${county || "County"} County Assessor (CAMA record)`;
  ...
  return {
    ...
    zoning: isSevier && cama?.ZONING
      ? fact(String(cama.ZONING).trim(), assessor)
      : fact(null, null, planning),

    jurisdiction:
      jurisdiction?.inCityLimits === null || jurisdiction?.inCityLimits === undefined
        ? fact(null, null, `${county || "County"} planning office`)
        : fact(
            jurisdiction.inCityLimits
              ? "Inside city limits — city zoning applies"
              : "Outside city limits — county zoning applies",
            "Sevierville GIS city-limits layer"
          ),

    overlayDistrict: jurisdiction?.overlayDistrict
      ? fact(jurisdiction.overlayDistrict, "Sevierville GIS overlay layer")
      : fact(null, null, null),
