...
// Sevier County's own ArcGIS Online org (referenced by TruTerra's web maps).
// Its parcel layer carries the CAMA schema but leaves most attributes null, so
// the Sevierville service above stays primary for zoning/buildings/utilities.
// What this org adds that nothing else does: live FEMA flood layers and city
// limits for all four Sevier municipalities.
const SEVIER_ORG = "https://services1.arcgis.com/Qu4yM4JJvNoC2GKw/arcgis/rest/services";
...
export const COUNTY_SOURCES = {
  sevier: {
    ...
    // Which municipality, not merely "a" city — zoning authority differs
    // between Sevierville, Pigeon Forge, Gatlinburg and Pittman Center.
    municipalities: [
      { url: `${SEVIER_ORG}/City_Limits_Sevier_County_TN/FeatureServer/0`, name: "Sevierville" },
      { url: `${SEVIER_ORG}/City_Limits_Sevier_County_TN/FeatureServer/1`, name: "Pigeon Forge" },
      { url: `${SEVIER_ORG}/City_Limits_Sevier_County_TN/FeatureServer/2`, name: "Gatlinburg" },
      { url: `${SEVIER_ORG}/City_Limits_Sevier_County_TN/FeatureServer/3`, name: "Pittman Center" },
    ],
    municipalitySource: "Sevier County GIS city-limits layers",
    flood: {
      layers: [
        { url: `${SEVIER_ORG}/FEMA_DATA/FeatureServer/2`, label: "100-year flood zone" },
        { url: `${SEVIER_ORG}/FEMA_DATA/FeatureServer/1`, label: "Regulatory floodway" },
      ],
      source: "Sevier County GIS (FEMA flood data)",
    },
  },
  blount: {
    label: "Blount",
...
// Kept local so parcel-intel has no import cycle with lib/hydro.
function hydroSummaryText(h) {
  if (!h) return null;
  if (h.crossesParcel) {
    const names = [...new Set((h.onParcel || []).map((x) => x.name).filter(Boolean))];
    return names.length ? `${names.join(", ")} crosses the parcel` : "A mapped stream crosses the parcel";
  }
...
export async function countyIntel(countyName, lat, lng) {
  ...
  if (cfg.municipalities) {
    jobs.push(
      Promise.all(
        cfg.municipalities.map((m) => pointFeature(m.url, lat, lng).then((a) => (a ? m.name : null)))
      ).then((hits) => {
        const found = hits.filter(Boolean);
        if (found.length) {
          out.municipality = found[0];
          out.municipalitySource = cfg.municipalitySource;
        } else if (hits.length) {
          out.municipality = null;
          out.municipalityChecked = true;
          out.municipalitySource = cfg.municipalitySource;
        }
      })
    );
  ...
  if (cfg.flood) {
    jobs.push(
      Promise.all(
        cfg.flood.layers.map((l) => pointFeature(l.url, lat, lng).then((a) => (a ? l.label : null)))
      ).then((hits) => {
        const found = hits.filter(Boolean);
        out.floodLive = {
          inFlood: found.length > 0,
          labels: found,
          checked: true,
          source: cfg.flood.source,
        };
      })
    );
  }
 ...
 */
export async function resolveParcel({ apn, address, county, state = "TN", hint = null }) {
  const issues = [];
  let lp = null;
  let method = hint?.lat != null ? "propertyradar" : null;

  // 1. Land Portal — OPTIONAL and OFF by default. It exhausted its daily quota
  //    mid-analysis and, when it was the sole source of coordinates, took the
  //    entire fact chain down with it. Slope/flood/utilities now come from USGS
  //    and TruTerra's own GIS exports. Set USE_LAND_PORTAL=1 to re-enable it as
  //    a supplementary source (TLP estimate, road frontage, legal description).
  const useLandPortal = process.env.USE_LAND_PORTAL === "1";
  if (useLandPortal && apn) {
    try {
  ...
  // Coordinates drive every downstream county lookup, so they must not depend on
  // a single vendor. PropertyRadar's coordinates take precedence and keep the
  // chain alive when Land Portal is rate-limited or out of export tokens.
  const lat = hint?.lat ?? lp?.property?.latitude ?? null;
  const lng = hint?.lng ?? lp?.property?.longitude ?? null;
  ...
  const radarApn = normalizeApn(hint?.apn);

  // Agreement between any two independent sources confirms parcel identity.
  const known = [
    ["PropertyRadar", radarApn],
    ["Land Portal", lpApn],
    ["TN parcel layer", tnApn],
  ].filter(([, v]) => v);
  ...
  if (known.length >= 2) {
    ...
    if (distinct.length === 1) {
      ...
      confidence = 55;
      issues.push(
        `APN mismatch between sources — ${known.map(([n, v]) => `${n} ${v}`).join(" vs ")}. Parcel identity NOT confirmed.`
      );
    }
  } else if (known.length === 1 || tn || lp) {
    confidence = 60;
    issues.push(
      `Only one source resolved this parcel (${known[0]?.[0] || (tn ? "TN parcel layer" : "unknown")}); no independent cross-validation.`
    );
  }
 ...
 */
export function buildFacts({ lp, intel, countyName, radar, terrain, soil, hydro }) {
  ...
  // Ownership: PropertyRadar reflects current record; county GIS lags (Sevier's
  // CAMA layer is tax year 2023). Where they disagree, PropertyRadar wins for
  // "who owns it now" and the lag itself is reported.
  const radarOwner = radar?.property?.Owner || null;
  const camaOwner = (cama?.OWNER || "").trim() || null;
  if (radarOwner && camaOwner) {
    const norm = (s) => String(s).toUpperCase().replace(/[^A-Z]/g, "");
    const a = norm(radarOwner);
    const b = norm(camaOwner);
    if (!a.includes(b.slice(0, 8)) && !b.includes(a.slice(0, 8))) {
      discrepancies.push(
        `OWNERSHIP LAG: PropertyRadar shows current owner "${radarOwner}"; the ${county || "county"} assessor record${cama?.TAXYR ? ` (tax year ${cama.TAXYR})` : ""} still shows "${camaOwner}". PropertyRadar reflects the more recent transfer — verify against the Register of Deeds.`
      );
    }
  }

  const port = radar?.owner
    ? {
        name: radar.owner.name,
        age: radar.owner.age,
        role: radar.owner.role,
        mailAddress: radar.owner.mailAddress,
        totalParcels: radar.totalParcels,
        otherParcels: radar.others || [],
        hasPhoneOnFile: radar.owner.hasPhoneOnFile,
        hasEmailOnFile: radar.owner.hasEmailOnFile,
      }
    : null;
  ...
  return {
    ...
    currentOwner: radarOwner
      ? fact(radarOwner, "PropertyRadar (current record)")
      : fact(null, null, `${county || "County"} Register of Deeds`),

    ownerPortfolio: port ? fact(port, "PropertyRadar person-level ownership linkage") : fact(null, null, null),

    freeAndClear:
      radar?.property?.isFreeAndClear != null
        ? fact(Boolean(radar.property.isFreeAndClear), "PropertyRadar")
        : fact(null, null, null),
    loanBalance:
      radar?.property?.TotalLoanBalance != null
        ? fact(radar.property.TotalLoanBalance, "PropertyRadar")
        : fact(null, null, null),
    distressScore:
      radar?.property?.DistressScore != null
        ? fact(radar.property.DistressScore, "PropertyRadar")
        : fact(null, null, null),
    radarId: radar?.property?.RadarID ? fact(radar.property.RadarID, "PropertyRadar") : fact(null, null, null),
    ...
    jurisdiction: intel?.municipality
      ? fact(`Inside ${intel.municipality} city limits — ${intel.municipality} zoning applies`, intel.municipalitySource)
      : intel?.municipalityChecked
      ? fact("Unincorporated county — county zoning applies", intel.municipalitySource)
    ...
    slopeAverage: terrain?.slope
      ? fact(terrain.slope.meanPercent, terrain.slope.source)
      : fact(lp?.slopeAverage ?? null, lp?.slopeAverage != null ? "Land Portal API v2" : null, "USGS 3DEP"),
    slopeMax: terrain?.slope
      ? fact(terrain.slope.maxPercent, terrain.slope.source)
      : fact(lp?.slopeMax ?? null, lp?.slopeMax != null ? "Land Portal API v2" : null, "USGS 3DEP"),
    slopeDegreesMean: terrain?.slope ? fact(terrain.slope.meanDegrees, terrain.slope.source) : fact(null, null, null),

    // Septic feasibility is the single most decisive soil fact for a rural
    // parcel with no public sewer — a "Very limited" rating can make a lot
    // effectively unbuildable, and no assessor or listing feed reports it.
    septicSuitability: soil?.septic?.rating
      ? fact(
          soil.septic.reasons?.length
            ? `${soil.septic.rating} — ${soil.septic.reasons.join(", ")}`
            : soil.septic.rating,
          `${soil.source} (${soil.septic.rule})`
        )
      : fact(null, null, "county environmental health / soil scientist"),
    soilSeries: soil?.mapUnit ? fact(soil.mapUnit, soil.source) : fact(null, null, null),
    soilDrainage: soil?.dominant?.drainage ? fact(soil.dominant.drainage, soil.source) : fact(null, null, null),
    farmlandClass: soil?.farmlandClass ? fact(soil.farmlandClass, soil.source) : fact(null, null, null),
    buildability: soil?.dwellings?.rating
      ? fact(
          soil.dwellings.reasons?.length
            ? `${soil.dwellings.rating} — ${soil.dwellings.reasons.join(", ")}`
            : soil.dwellings.rating,
          `${soil.source} (dwellings without basements)`
        )
      : fact(null, null, null),

    // A creek near the parcel compounds a poor septic rating (TN imposes
    // setbacks from streams) and can quietly remove usable acreage.
    surfaceWater: hydro
      ? fact(hydroSummaryText(hydro), hydro.source)
      : fact(null, null, null),
    streamCrossesParcel: hydro ? fact(hydro.crossesParcel ? "Yes" : "No", hydro.source) : fact(null, null, null),

    sewerDistanceFeet: terrain?.sewer
      ? fact(terrain.sewer.feet, terrain.sewer.source)
      : fact(null, null, "county utility district"),
    hydrantDistanceFeet: terrain?.hydrant
      ? fact(terrain.hydrant.feet, terrain.hydrant.source)
      : fact(null, null, "local fire district / water utility"),
    roadFrontage: fact(lp?.roadFrontage ?? null, "Land Portal API v2"),
    landLocked: fact(lp?.landLocked ?? null, "Land Portal API v2"),
    floodplain100yr: intel?.floodLive?.checked
      ? fact(
          intel.floodLive.inFlood
            ? `Within mapped ${intel.floodLive.labels.join(" and ")}`
            : "Outside mapped FEMA flood zones",
          intel.floodLive.source
        )
      : terrain?.flood
      ? fact(terrain.flood.inFloodplain ? "Within mapped 100-year floodplain" : "Outside mapped 100-year floodplain", terrain.flood.source)
      : fact(null, null, "FEMA FIRM panel"),
    floodCoveragePct: fact(lp?.femaCoverPercentage ?? null, lp?.femaCoverPercentage != null ? "FEMA via Land Portal API v2" : null),
